...
const config = {
  ...
  directory: 'gfw-mapbuilder/',
  projectDir: 'gfw-mapbuilder', //the name of the project folder on your machine
  buildCommand: 'npm run build-lib', //switch to npm or whatever you use to run build-lib command
  get awsS3Command() {
    return `aws s3 sync --acl public-read --include --profile blueraster 'libBuild/' libBuild s3://${
      this.bucket
...
function main() {
  ...
  const testURL = `https://${config.bucket}${config.directory}${branchName}/external.html`;
